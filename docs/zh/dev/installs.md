...
本页说明 dsh 内核、Node、npm 的下载与安装：版本化目录、下载器、取消、解压阶段与 npm 缓存。

## 版本化安装目录

Node、npm、内核都按版本分开存放，多版本并存：

```text
<配置目录>/
├─ node/<版本>/           Node 运行时（含自带的 npm）
├─ npm/<版本>/package/    应用代管的内置 npm
└─ kernel/<版本>/         内置内核（node_modules/@deepseek-ai/dsh）
```

每个根目录下的 `.active` 记录当前生效版本。核心 API 在 `main/kernel/installs.ts`：

| 函数 | 作用 |
|---|---|
| `installRoot(kind)` / `versionDir(kind, v)` | 拼路径 |
| `resolveActive(kind)` | 取生效版本；无指针时回退到最新**完整**版本并写回 |
| `isVersionComplete(kind, v)` | 关键文件是否就位（node 要 `node.exe`，npm 要 `package/bin/npm-cli.js`，内核要 dsh `package.json`） |
...
`main/kernel/downloader.ts` 的 `downloadFile`：
...
`main/kernel/cancel.ts` 提供单活动令牌：`beginCancelable()` / `cancelActive()` / `isAbortError()` / `CANCELED_MESSAGE`。
...
- **npm**：`npmRunner.ensureBundledNpm` 从 registry 下载 tgz → 解压到 `npm/<版本>/`；
- **内核**：`kernel.installTo` 用选定 npm 通过 `--prefix` 装进 `kernel/<版本>/`，再写 `.active`；
